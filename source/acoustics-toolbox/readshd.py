...
def readshd(filename=None, xs=None, ys=None, freq=None):
    ...
    if PlotType == 'rectilin  ':
        pressure = np.zeros((Ntheta, Nsz, Nrz, Nrr)) + 1j * \
            np.zeros((Ntheta, Nsz, Nrz, Nrr))
        Nrcvrs_per_range = Nrz
    elif PlotType == 'irregular ':
        pressure = np.zeros((Ntheta, Nsz,   1, Nrr)) + 1j * \
            np.zeros((Ntheta, Nsz,   1, Nrr))
        Nrcvrs_per_range = 1
    else:
        pressure = np.zeros((Ntheta, Nsz, Nrz, Nrr)) + 1j * \
            np.zeros((Ntheta, Nsz, Nrz, Nrr))
    ...
    if np.isnan(xs):
        ...
        for itheta in range(Ntheta):
            for isz in range(Nsz):
                for irz in range(Nrcvrs_per_range):
                    recnum = 10 + ifreq*Ntheta*Nsz*Nrcvrs_per_range + itheta * \
                        Nsz*Nrcvrs_per_range + isz * Nrcvrs_per_range + irz
                    ...
                    pressure[itheta, isz, irz, :] = temp[indexes] + \
                        1j*temp[indexes+1]
        ...
        for itheta in range(Ntheta):
            for isz in range(Nsz):
                for irz in range(Nrcvrs_per_range):
                    recnum = 10 + idxX * Nsy * Ntheta * Nsz * Nrcvrs_per_range + idxY * Ntheta * Nsz * \
                        Nrcvrs_per_range + itheta * Nsz * Nrcvrs_per_range + isz * Nrcvrs_per_range + irz
                    # Move to end of previous record
                    status = fid.seek(recnum * 4 * recl)
                    if (status == -1):
        ...
        fid.close()
    geometry = {"zs": zs, "f": freqVec, "thetas": theta,
                "rarray": rarray, "zarray": zarray}
